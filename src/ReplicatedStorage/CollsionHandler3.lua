...
function  collisions.entityvsterrain(entity,velocity)
    ...
    for i =1,3,1 do
        ...
        if MinTime <1 and false then
            --print(refunction.convertPositionto(normal,"string"))
...
function collisions.entityvsterrainloop(entity,velocity)
    ...
    local min ={
        position[1]-hitbox.x/2+(velocity[1] <0 and velocity[1] or 0)   ,
        position[2]-hitbox.y/2+(velocity[2] <0 and velocity[2] or 0), 
        position[3]-hitbox.z/2+(velocity[3] <0 and velocity[3] or 0)   
    }
    local max ={
        position[1]+hitbox.x/2 +(velocity[1] >0 and velocity[1] or 0),
        position[2]+hitbox.y/2+(velocity[2] >0 and velocity[2] or 0), 
        ...
        for y = min[2],max[2],1 do
            for z = min[3],max[3],1 do
                ...
                if block  then
                    ...
                    local collisiontime,newnormal = collisions.SweaptAABB(position,a,{hitbox.x,hitbox.y,hitbox.z},{4,4,4},nil,nil,velocity,mintime)
